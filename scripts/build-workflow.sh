...
#
# Steps:
#   1. Build cmd/quick-txt-save-alfred as a universal (amd64+arm64)
...
GOOS=darwin GOARCH=amd64 go build -o "${BUILD_DIR}/quick-txt-save-alfred-amd64" ./cmd/quick-txt-save-alfred
GOOS=darwin GOARCH=arm64 go build -o "${BUILD_DIR}/quick-txt-save-alfred-arm64" ./cmd/quick-txt-save-alfred
lipo -create -output "${BUILD_DIR}/quick-txt-save-alfred" \
  "${BUILD_DIR}/quick-txt-save-alfred-amd64" \
  "${BUILD_DIR}/quick-txt-save-alfred-arm64"
rm "${BUILD_DIR}/quick-txt-save-alfred-amd64" "${BUILD_DIR}/quick-txt-save-alfred-arm64"
chmod +x "${BUILD_DIR}/quick-txt-save-alfred"
lipo -info "${BUILD_DIR}/quick-txt-save-alfred"

if [ -n "${CODESIGN_IDENTITY:-}" ]; then
  echo "→ Signing entrypoint binary (${CODESIGN_IDENTITY})"
  codesign --force --options runtime --timestamp --sign "$CODESIGN_IDENTITY" \
    "${BUILD_DIR}/quick-txt-save-alfred"
  codesign --verify --strict --verbose=2 "${BUILD_DIR}/quick-txt-save-alfred"

  if [ -n "${NOTARY_KEY_ID:-}" ]; then
    "${REPO_ROOT}/scripts/notarize-binary.sh" "${BUILD_DIR}/quick-txt-save-alfred"
  fi
else
