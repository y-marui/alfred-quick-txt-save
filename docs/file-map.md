...
| ファイル | 役割 | 主な依存先 |
|---|---|---|
| `cmd/quick-txt-save-alfred/main.go` | Alfred が実行する唯一のバイナリ。サブコマンド (`list`/`write [path]`) ディスパッチのみ | `internal/quicksavecmd`、`internal/quicksave`、`internal/scriptfilter` |
...
| `internal/quicksave/save.go` | ファイル書き込み・macOS 通知（保存成功/クリップボード空/書き込み失敗）。クリップボードの読み取りは `workflow/info.plist` の Arguments and Variables ノード（`{clipboard}` → `text` 変数）が担い、このパッケージは `text` を受け取るのみ。`path` が空（Universal Action 経由）のときは `ResolvePath("")` で自動生成名を使う | — |
